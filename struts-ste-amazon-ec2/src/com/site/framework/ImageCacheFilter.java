...
public class ImageCacheFilter implements Filter {
    ...
    private static final Logger log = Logger.getLogger(ImageCacheFilter.class);
    ...
    private void parseHeader(String header) {
        ...
        log.info("Putting Map : KEY" + headerName + " VALUE : " + header.substring(header.indexOf(":") + 1));
        if (!headersMap.containsKey(headerName)) {
            if (null != headerName && null != header.substring(header.indexOf(":") + 1)) {
                headersMap.put(headerName, header.substring(header.indexOf(":") + 1));
                headersMap.put(CACHE_CONTROL, MAX_AGE);
            }
        }
    }
}
