...
AmCharts.ready(function () {
    ...
    $("#div1").load("fetchChartDataAJAXHtml5ChartData.action #ASIdateList",function(responseTxt,statusTxt,xhr){
        if(statusTxt=="success")
            var ajaxResponse = responseTxt;
        var responseObjK = jQuery.parseJSON(ajaxResponse);
...
function createStockChart() {
    ...
    dataSet1 = createDataSource("All Share Index", json_chartDataAJAX_ASI, chartDataAJAX_ASI);

    dataSet2 = createEmptyDataSource("MPI Price Index", json_chartDataAJAX_MPI, chartDataAJAX_MPI);
    dataSet3 = createEmptyDataSource("SNP20 Index", json_chartDataAJAX_SNP20, chartDataAJAX_SNP);
    ...
    $("#div1").load("fetchChartDataAJAXHtml5ChartData.action #MPIdateList",function(responseTxt,statusTxt,xhr){
        if(statusTxt=="success")
            var ajaxResponse = responseTxt;
        var responseObjK = jQuery.parseJSON(ajaxResponse);
        json_chartDataAJAX_MPI = responseObjK.MPIdateList;
        console.log("MPI OBJECT " + json_chartDataAJAX_MPI);

        dataSet3 = createDataSource("MPI Index", json_chartDataAJAX_MPI, chartDataAJAX_MPI);
        chart.validateData()

    });

    $("#div1").load("fetchChartDataAJAXHtml5ChartData.action #SNP20dateList",function(responseTxt,statusTxt,xhr){
        if(statusTxt=="success")
            var ajaxResponse = responseTxt;
        var responseObjK = jQuery.parseJSON(ajaxResponse);
        json_chartDataAJAX_SNP20 = responseObjK.SNP20dateList;
        console.log("SNP OBJECT " + json_chartDataAJAX_SNP20);
        chart.validateData()
